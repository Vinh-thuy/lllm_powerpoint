...
def process_prompt_line(prompt_line):
    """
    Traite une ligne de prompt et met à jour la roadmap
    
    Args:
        prompt_line (str): Une ligne de prompt à traiter
    
    Returns:
        dict: Informations de la tâche traitée, ou None en cas d'erreur
    """
...
if __name__ == "__main__":
    # Si un argument est passé, traiter ce prompt spécifique
    if len(sys.argv) > 1:
        prompt_text = ' '.join(sys.argv[1:])
        process_prompt_line(prompt_text)
    else:
        # Sinon, exécuter le main() par défaut
        main()